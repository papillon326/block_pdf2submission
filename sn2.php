...
$inputfolder = "/home/ec2-user/pdfs/";  // Where the PDF files are FIXME: configurable or upload by form

$tmpd = $CFG->dataroot . "/temp/";  // The directory for temporary files FIXME:
$warning_mailto = $CFG->supportemail; // FIXME:

require_login();

$useridarr = array();
function qrsn2userid($assignno, $qrsn) {
    global $tmpd, $useridarr;  // FIXME: why use global vars, what is $useridarr?
...
echo '<div style="background-color:ivory;padding:1em;">';
...
foreach($dirs0 as $dir0){
  if ($dir0 != "." && $dir0 != "..") {
    $selected = ($dir0 == $_POST['cfolder']) ? 'selected="selected"' : '';
    p("<option value=\"{$dir0}\" {$selected}>{$dir0}</option>");
  }
}
echo '</select>';
...
$currentfolder= $inputfolder."/".s($_POST['cfolder'], TRUE)."/";
...
while(($ent3 = $dir3->read()) !== false){
    if(preg_match('/^(\w+).pdf.QRSN$/', $ent3, $match)){
        ...
        if (isset($_POST[$key1]) && $_POST[$key1] != "") {
            // FIXME: using %idnumber% may cause serious problem...
        ...
        if ( !isset($users) || (isset($users) && count($users)!=1) ){
            echo count($users);
            printf("<div style=\"background-color:%s;padding:1em;\">", ($co%2==0) ? "red" : "salmon");  // TODO: use CSS
        }else{
            printf("<div style=\"background-color:%s;padding:1em;\">", ($co%2==0) ? "#aa88ee" : "#ddbbff");  // TODO: use CSS
        }
        
        preg_match('/QRSN(\d+)/', $userid, $match2);
        $qnum= $match2[1];
        printf("asno = %d", $asno);
        printf(", qrsn = %s", $qnum);
        ...
        printf("<div style=\"{$dheight}\"><img style=\"{$clip}\" width=600px src=\"data:image/gif;base64,%s\"></div>",
          base64_encode(file_get_contents($currentfolder.$filenamebase.".jpg")));
